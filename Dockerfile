...
RUN apt-get update && \
    apt-get install -y postgresql metasploit-framework armitage openvpn supervisor && \
...
RUN sed -i 's/CN=Armitage Hacker/CN=YOUR CN/g' /usr/share/armitage/teamserver
RUN sed -i 's/OU=FastAndEasyHacking/OU=YOUR OU/g' /usr/share/armitage/teamserver

# autoconnect to vpn (.conf files in /etc/openvpn)
RUN sed -i 's/#AUTOSTART="all"/AUTOSTART="all"/g' /etc/default/openvpn

RUN sed -i 's#/var/lib/postgresql/10/main#/var/lib/postgresql/data#g' /etc/postgresql/10/main/postgresql.conf && \ 
